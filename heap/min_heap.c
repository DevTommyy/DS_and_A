...
int main() {
    ...
    heap->current = 0; // Initialize heap
    ...
    int val_to_find = 20;
    if (contains(heap, val_to_find)) {
        printf("%d is in the heap.\n", val_to_find);
    } else {
        printf("%d is not in the heap.\n", val_to_find);
    }

    // Testing index_of function
    int index = index_of(heap, 25);
    if (index != -1) {
        printf("Index of 25: %d\n", index);
    } else {
        printf("25 not found in the heap.\n");
    }

    free(heap); // Free dynamically allocated memory
    return 0;
}
