...
/// # Params
///
/// - `old_map` the old `HashMap` to map_rehash
void map_rehash(HashMap* old_map) {
...
/// # Returns
///
/// `true` if the `HashMap` needs a map_rehash, `false` otherwise.
bool map_needs_rehashing(HashMap* map) {
...
LinkedList map_remove_key(HashMap* map, int key) {
    uint32_t hashedKey = fnv1a_hash(&key, sizeof(key));
    int entry = hashedKey % map->capacity;
